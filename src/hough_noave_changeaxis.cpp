...
void scanCallback(const sensor_msgs::LaserScan msg)
{
  ...
  /*
	...
	*/
		line(img, Point(_x1[i]+200,_y2[i]+200), Point(_x2[i]+200,_y1[i]+200), Scalar(255,0+i*10,0), 3);
  }
...
void hough_transform(float ave_laser[],int &x1,int &x2,int &y1,int &y2,float &r_save){
	...
	float acc_num[41];
	int ticket_max=0;
	float a=0,b=0,x0,y0,x[44]={},y[44]={};
	int i ,j,k,out=0;
	x1=0,x2=0,y1=0,y2=0;
	//float x1,x2,y1,y2;
	
	for(i=682-cnt*42;i>=640-cnt*42;i--){
	...
	if(out == 0){
	for(i=682-cnt*42;i>=640-cnt*42;i--){
		x[683-(i+cnt*42)] = ave_laser[i]*cos((598-i)*0.3515*pi/180);
		y[683-(i+cnt*42)] = ave_laser[i]*sin((598-i)*0.3515*pi/180);
		//cout << "x_up[" << 683-(i+cnt*42) << "]:" << ave_laser[i]*cos((598-i)*0.3515*pi/180) << endl;
		//cout << "x[" << 683-(i+cnt*42) << "]:" << x[683-(i+cnt*42)] << endl;
